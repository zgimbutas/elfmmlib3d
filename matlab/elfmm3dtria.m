function [U]=elfmm3dtria(iprec,rlam,rmu,nsource,triaflat,trianorm,source,ifcharge,charge,ifdipole,dipstr,ifpot,ifstrain,ntarget,target,ifpottarg,ifstraintarg)
%ELFMM3DTRIA Elastostatic triangle target FMM in R^3.
%
%
% [U]=ELFMM3DTRIA(IPREC,RLAM,RMU,NSOURCE,TRIAFLAT,TRIANORM,SOURCE,...
...
if( ntarget == 0 ) 
mex_id_ = 'elfmm3dtria(io int[x], i int[x], i double[x], i double[x], i double[], i double[xx], i int[x], i double[xx], i int[x], i double[], i int[x], i double[], i int[x], io double[], i int[x], io double[])';
[ier, pot, strain] = elfmm3d_r2013a(mex_id_, ier, iprec, rlam, rmu, triaflat, trianorm, nsource, source, ifcharge, charge, ifdipole, dipstr, ifpot, pot, ifstrain, strain, 1, 1, 1, 1, 3, nsource, 1, 3, nsource, 1, 1, 1, 1);
else
mex_id_ = 'elfmm3dtriatarg(io int[x], i int[x], i double[x], i double[x], i double[], i double[xx], i int[x], i double[xx], i int[x], i double[], i int[x], i double[], i int[x], io double[], i int[x], io double[], i int[x], i double[xx], i int[x], io double[], i int[x], io double[])';
[ier, pot, strain, pottarg, straintarg] = elfmm3d_r2013a(mex_id_, ier, iprec, rlam, rmu, triaflat, trianorm, nsource, source, ifcharge, charge, ifdipole, dipstr, ifpot, pot, ifstrain, strain, ntarget, target, ifpottarg, pottarg, ifstraintarg, straintarg, 1, 1, 1, 1, 3, nsource, 1, 3, nsource, 1, 1, 1, 1, 1, 3, ntarget, 1, 1);
end
