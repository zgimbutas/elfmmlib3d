function [U]=elh3dpartdirecttime(rlam,rmu,ms,nsource,source,ifcharge,charge,ifdipole,dipstr,dipvec,ifpot,ifstrain,mt,ntarget,target,ifpottarg,ifstraintarg)
%ELH3DTPARTDIRECT Elastostatic particle interaction in R^3, direct evaluation.
%
...
%
%
% [U]=ELH3DPARDIRECTTTARG(RLAM,RMU,NSOURCE,SOURCE,...
%         IFCHARGE,CHARGE,IFDIPOLE,DIPSTR,DIPVEC,IFPOT,IFSTRAIN);
%
% [U]=ELH3DPARTDIRECT(RLAM,RMU,NSOURCE,SOURCE,...
%         IFCHARGE,CHARGE,IFDIPOLE,DIPSTR,DIPVEC,IFPOT,IFSTRAIN,...
...
mex_id_ = 'elh3dpartdirecttargtime(i double[x], i double[x], i int[x], i int[x], i double[xx], i int[x], i double[], i int[x], i double[], i double[xx], i int[x], io double[], i int[x], io double[], i int[x], i int[x], i double[xx], i int[x], io double[], i int[x], io double[])';
[pot, strain, pottarg, straintarg] = elfmm3d_r2013a(mex_id_, rlam, rmu, ms, nsource, source, ifcharge, charge, ifdipole, dipstr, dipvec, ifpot, pot, ifstrain, strain, mt, ntarget, target, ifpottarg, pottarg, ifstraintarg, straintarg, 1, 1, 1, 1, 3, nsource, 1, 1, 3, nsource, 1, 1, 1, 1, 3, ntarget, 1, 1);
