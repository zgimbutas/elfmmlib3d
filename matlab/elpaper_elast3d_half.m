...
tic
[U]=elhfmm3dpart(iprec,rlam,rmu,nsource,source,ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,ifpot,ifstrain);
total_time=toc;
...
k = 500;
if( nsource < 10000), k = 5; end;
if( nsource > 100000), k = 5000; end;

ms = nsource / k;
tic
[F]=elh3dpartdirecttime(rlam,rmu,ms,nsource,source,ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,ifpot,ifstrain);
total_time=toc;
...
if( ifstrain ), U.strain=U.strain(:,1:ms); end

if( ifpot ), F.pot=F.pot(:,1:ms); end
if( ifstrain ), F.strain=F.strain(:,1:ms); end
...
if( ifstrain ),
rel_error_strain = norm(U.strain - F.strain,2)/norm(F.strain,2);
end
...
if( ifstrain ),
  fprintf('%5d & %2d & %5.3f & %5.3f & %6.3e \\\\ \n', nsource, iprec, total_time_fmm, total_time_direct, rel_error_strain );
end
