function [U]=elfmm3dpart_matlab2(iprec,rlam,rmu,nsource,source,ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,ifpot,ifgrad,ntarget,target,ifpottarg,ifgradtarg)
...
if( ifpot == 1 ),
   pot(j,:) = pot(j,:) + (2-c1)*H.pot;
   pot(1,:) = pot(1,:) + c1*source(j,:).*H.fld(1,:);
   pot(2,:) = pot(2,:) + c1*source(j,:).*H.fld(2,:);
   pot(3,:) = pot(3,:) + c1*source(j,:).*H.fld(3,:);
   pre = pre + 2*H.fld(j,:);
end
...
F.grad(1,1,:) = H.hess(1,:).*source(j,:)*c1;
F.grad(2,2,:) = H.hess(2,:).*source(j,:)*c1;
F.grad(3,3,:) = H.hess(3,:).*source(j,:)*c1;
F.grad(1,2,:) = H.hess(4,:).*source(j,:)*c1;
F.grad(1,3,:) = H.hess(5,:).*source(j,:)*c1;
F.grad(2,3,:) = H.hess(6,:).*source(j,:)*c1;
F.grad(2,1,:) = H.hess(4,:).*source(j,:)*c1;
F.grad(3,1,:) = H.hess(5,:).*source(j,:)*c1;
F.grad(3,2,:) = H.hess(6,:).*source(j,:)*c1;

F.grad(1:3,j,:) = reshape(F.grad(1:3,j,:),3,nsource) - H.fld*c1;
F.grad(j,1:3,:) = reshape(F.grad(j,1:3,:),3,nsource) + H.fld*(2-c1);
...
if( ifpottarg == 1 ),
   pottarg(j,:) = pottarg(j,:) + (2-c1)*H.pottarg;
   pottarg(1,:) = pottarg(1,:) + c1*target(j,:).*H.fldtarg(1,:);
   pottarg(2,:) = pottarg(2,:) + c1*target(j,:).*H.fldtarg(2,:);
   pottarg(3,:) = pottarg(3,:) + c1*target(j,:).*H.fldtarg(3,:);
...
F.gradtarg(1,1,:) = H.hesstarg(1,:).*target(j,:)*c1;
F.gradtarg(2,2,:) = H.hesstarg(2,:).*target(j,:)*c1;
F.gradtarg(3,3,:) = H.hesstarg(3,:).*target(j,:)*c1;
F.gradtarg(1,2,:) = H.hesstarg(4,:).*target(j,:)*c1;
F.gradtarg(1,3,:) = H.hesstarg(5,:).*target(j,:)*c1;
F.gradtarg(2,3,:) = H.hesstarg(6,:).*target(j,:)*c1;
F.gradtarg(2,1,:) = H.hesstarg(4,:).*target(j,:)*c1;
F.gradtarg(3,1,:) = H.hesstarg(5,:).*target(j,:)*c1;
F.gradtarg(3,2,:) = H.hesstarg(6,:).*target(j,:)*c1;

F.gradtarg(1:3,j,:) = reshape(F.gradtarg(1:3,j,:),3,ntarget) - H.fldtarg*c1;
F.gradtarg(j,1:3,:) = reshape(F.gradtarg(j,1:3,:),3,ntarget) + H.fldtarg*(2-c1);
...
if( ifdouble == 1 ),
  ...
  ifcharge = 1;
  charge = charge + (sigma_dl(1,:).*sigma_dv(1,:) + ...
            sigma_dl(2,:).*sigma_dv(2,:) + ...
            sigma_dl(3,:).*sigma_dv(3,:))*c2;
...
% compute strains
if( ifgrad == 1 ), grad = (grad+grad')/2; end
if( ifgradtarg == 1 ), gradtarg = (gradtarg+gradtarg')/2; end
