...
C$        t2=omp_get_wtime()
        call prin2('time in d3tlsts=*',t2-t1,1)
c
...
c
        kid=collkids(i)
ccc        call d3tifint(corners(1,1,kid),corners(1,1,ibox),ifinter)
        call d3tifint2(boxes(1,kid),boxes(1,ibox),ifinter)
c
        if(ifinter .eq. 1)
...
c       to the daddy
c
ccc        call d3tifint(corners(1,1,ibox),corners(1,1,jbox),ifinter)
        call d3tifint2(boxes(1,ibox),boxes(1,jbox),ifinter)
c
...
c       
        allocate( is(8,maxboxes),ns(8,maxboxes),
     $      istarg(8,maxboxes),nstarg(8,maxboxes),son_idx(maxboxes))
...
c
c       ... optionally, adjust the computational box center, 
c       force x=0, y=0, or z=0
c
c       ... find the maximum distance of the first coordinate
c
ccc        distx=max(abs(xmin),abs(xmax))
ccc        center0(1)=0
ccc        sizex=2*distx
ccc        if(sizex .gt. size) size=sizex
c
c       ... find the maximum distance of the second coordinate
c
ccc        disty=max(abs(ymin),abs(ymax))
ccc        center0(2)=0
ccc        sizey=2*disty
ccc        if(sizey .gt. size) size=sizey
c
c       ... find the maximum distance of the third coordinate
c
        distz=max(abs(zmin),abs(zmax))
        center0(3)=0
        sizez=2*distz
        if(sizez .gt. size) size=sizez
c
...
C$OMP$PRIVATE(ii,jj,kk,iiz,nz,iiztarg,nztarg,center)
...
c
        call d3tsepa1(center,z(1,iiz),iz(iiz),nz,iwork,
     1    is(1,idad),ns(1,idad))
c
        call d3tsepa1(center,ztarg(1,iiztarg),iztarg(iiztarg),
     $     nztarg,iwork,istarg(1,idad),nstarg(1,idad))
c
...
C$OMP$PRIVATE(ii,jj,kk,iiz,nz,iiztarg,nztarg)
C$OMP$PRIVATE(idadson,ison,i,lll,iison,jjson,kkson)
cccC$OMP$SCHEDULE(DYNAMIC)
C$OMP$NUM_THREADS(1)
...
c
        ...
        boxes(5,ison)=idad        
c
        boxes(14,ison)=is(i,idad)+iiz-1
        boxes(15,ison)=ns(i,idad)
c
        boxes(16,ison)=istarg(i,idad)+iiztarg-1
        boxes(17,ison)=nstarg(i,idad)
c
        if( ns(i,idad) .le. 0 ) boxes(18,ison)=0
        if( nstarg(i,idad) .le. 0 ) boxes(19,ison)=0
        if( ns(i,idad) .gt. 0 ) boxes(18,ison)=1
        if( nstarg(i,idad) .gt. 0 ) boxes(19,ison)=1
        boxes(20,ison)=0
c
...
C$OMP$PRIVATE(i,level,side,side2,ii,jj,kk,center)   
        do 1400 i=1,nboxes
        level=boxes(1,i)
        side=size/2**level
        side2=side/2
...
ccc        thresh=cent(1)
        itype=3
        thresh=cent(3)
        call d3tsepa0ipt(z,iz,n,itype,thresh,iwork,n1234)
        n5678=n-n1234
c
...
c
        ...
        thresh=cent(2)
        if(n1234 .ne. 0) 
     1    call d3tsepa0ipt(z,iz,n1234,itype,thresh,iwork,n12)
        n34=n1234-n12
c
        if(n5678 .ne. 0) 
     1    call d3tsepa0ipt(z(1,n1234+1),iz(n1234+1),
     $     n5678,itype,thresh,iwork,n56)
        n78=n5678-n56
c
...
ccc     thresh=cent(3)
        ...
        thresh=cent(1)
        if(n12 .ne. 0) 
     1    call d3tsepa0ipt(z,iz,n12,itype,thresh,iwork,n1)
        n2=n12-n1
c
        if(n34 .ne. 0) 
     1    call d3tsepa0ipt(z(1,n12+1),iz(n12+1),
     $     n34,itype,thresh,iwork,n3)
        n4=n34-n3
c
        if(n56 .ne. 0) 
     1    call d3tsepa0ipt(z(1,n1234+1),iz(n1234+1),
     $     n56,itype,thresh,iwork,n5)
        n6=n56-n5
c
        if(n78 .ne. 0) 
     1    call d3tsepa0ipt(z(1,n1234+n56+1),iz(n1234+n56+1),
     $     n78,itype,thresh,iwork,n7)
        n8=n78-n7
c
c
...
c
c
        subroutine d3tsepa0ip(z,iz,n,itype,thresh,iwork,n1)
        implicit real *8 (a-h,o-z)
        real *8 z(3,*)
        integer iz(*),iwork(*)
ccc        save
...
c
c
        subroutine d3tsepa0ipt(z,iz,n,itype,thresh,iwork,n1)
        implicit real *8 (a-h,o-z)
        real *8 z(3,*)
        integer iz(*),iwork(*)
ccc        save
...
c        do i=1,n1
c        if( z(itype,i) .gt. thresh ) then
c        write(*,*) i1,i2,n1
c        write(*,*) i,n, '+', z(itype,i)
c        pause
c        endif
c        enddo

c        do i=n1+1,n
c        if( z(itype,i) .le. thresh ) then
c        write(*,*) i1,i2,n1
c        write(*,*) i,n, '-', z(itype,i)
c        pause
c        endif
