...
c
c     lfmm3dmindlintarg - Laplace FMM in R^3: evaluate all pairwise
c         particle interactions (ignoring self-interaction) +
c         interactions with targets
c
c     l3dmindlindirect - direct interactions in R^3: evaluate interactions
...
c
c
      subroutine lfmm3dmindlintriatarg(ier,iprec,nsource,triaflat,
     $     trianorm,source,rlame,ifsingle,sigma_sl,ifdouble,dipstr,
     $     dipvec,ntarget,target,ifptfrctarg,ptfrctarg,
     $     ifhesstarg,hesstarg)
c       
...
c       triaflat    :   triangle coordinates
c       trianorm    :   triangle normals
...
c
      ...
      integer itriaflatsort,ltriaflatsort
      integer itrianormsort,ltrianormsort
      integer lsourcesort,ltargetsort,lused,lwlists,nbox
      integer nboxes,nchwork,nexpback,nexpe,nexptot,nexptotp
      integer nlams,nlev,nmax,nskel,nthmax,nthmaxp,ntot,nnodesmax
...
c
      nnodesmax=100
      nchwork = max(nbox*nnodesmax,4*nmax*nmax)
...
c
      call l3dtriareorderv2(nsource,source,ifsingle,sigma_sl,
     $     wlists(iisource),ifdouble,dipstr,dipvec,
     1     w(isourcesort),w(ichargesort),w(idipvecsort),w(idipstrsort)) 
c       
      call l3dreordertria(nsource,wlists(iisource),triaflat,
     1       w(itriaflatsort),trianorm,w(itrianormsort))
c
...
c
      call l3dtriamindlinalloc(wlists(iwlists),iaddr,nboxes,lmptot,
     1       nterms,nexptotp)
c
...
c
      ...
      ifevalloc=1
c
      call lfmm3dmindlintriatargmain(ier,iprec,
     $   ifevalfar,ifevalloc,nsource,
     $   w(itriaflatsort),w(itrianormsort),
...
c
        subroutine lfmm3dmindlintriatargmain(ier,iprec,
     $     ifevalfar,ifevalloc,nsource,triaflatsort,trianormsort,
     $     sourcesort,isource,rlame,ifsingle,chargesort,
     $     ifdouble,dipstrsort,dipvecsort,ntarget,
...
C
        implicit none
        integer ier,iprec,ifevalfar,ifevalloc,nsource,nqtri
        ...
        real *8 triaflatsort(3,3,nsource)
        real *8 trianormsort(3,nsource)
        real *8 rlams(nlams)
        real *8 whts(nlams)
...
c
c
c       WARNING: nnodesmax is set to 100, nqtri parameter should generate
c       no more than 100 nodes per triangle in forming mindlin multipoles
c
        nqtri = 1
        if( iprec .eq. -2 ) then
        nqtri=2
...
c
c
c       ... find the first box for which the multipole expansion needs
c       to be formed (set level3box = nboxes+1, if no multipoles)
c
            if( nlev+1 .ge. 3 ) then
            level3box=laddr(1,3)
            else
...
C$OMP$PRIVATE(ier,i,j,ptemp,ftmp) 
C$OMP$SCHEDULE(DYNAMIC)
cccC$OMP$NUM_THREADS(1) 
ccc        do 1200 ibox=1,nboxes
        do 1200 ibox=level3box,nboxes
...
c
	       ...
	       radius = sqrt(radius)
c
               call l3dtriaformmp_mindlinb_trunc
     $         (nqtri,scale(level),rlame,
     $          triaflatsort(1,1,box(14)),trianormsort(1,box(14)),
...
c
               call l3dtriaformmp_mindlinc_trunc
     $         (nqtri,scale(level),rlame,
     $          triaflatsort(1,1,box(14)),trianormsort(1,box(14)),
     $          sourcesort(1,box(14)),ifsingle,chargesort(1,box(14)),
     $          ifdouble,dipstrsort(1,box(14)),dipvecsort(1,box(14)),
...
c
c
                  call l3dtriaformtaintkerbf90(ier,nqtri,scale(level1),
     $               triaflatsort(1,1,box(14)),trianormsort(1,box(14)),
     ...
     $                    nterms(level1))
c
                  call l3dtriaformtaintkercf90(ier,nqtri,scale(level1),
     $               triaflatsort(1,1,box(14)),trianormsort(1,box(14)),
     1               sourcesort(1,box(14)),ifsingle,
     1               chargesort(1,box(14)),ifdouble,
     $               dipstrsort(1,box(14)),dipvecsort(1,box(14)),
     ...
     $                    nterms(level1))
                  ...
                  call lfmm3dmindlintria_direct(nqtri,box,box1,rlame,
     $                  triaflatsort,trianormsort,
...
c       
            if (level .ge. 2) then
               do jj = box(16),box(16)+box(17)-1
                  call mindlintaevalb_trunc(scale(level),center0,
     $                 rmlexp(iaddr(1,ibox)),nterms(level),
     ...
     $                    hesstmp(kk,ll)
                  ...
                  call mindlintaevalc_trunc(scale(level),center0,
     $                 rmlexp(iaddr(2,ibox)),rmlexp(iaddr(3,ibox)),
     $                 nterms(level),targetsort(1,jj),
     $                 ftmp,ifhesstarg,hesstmp,
     $                 scarray_local,wlege,nlege)
...
c         call lfmm3dmindlintria_direct_self(box,rlame,
c     $        triaflatsort,trianormsort,sourcesort,
c     $        ifsingle,chargesort,ifdouble,dipstrsort,dipvecsort,
c     $        targetsort,ifptfrctarg,ptfrctarg,ifhesstarg,hesstarg)
c
c
c       ... retrieve list #1
c
...
c    
               call lfmm3dmindlintria_direct(nqtri,box1,box,rlame,
...
c
c
      subroutine lfmm3dmindlintria_direct_self(box,rlame,
     $     triaflat,trianorm,source,
     $     ifsingle,charge,ifdouble,dipstr,dipvec,
...
c
c
      subroutine lfmm3dmindlintria_direct(nqtri,box,box1,rlame,
     $     triaflat,trianorm,
...
c
        ...
        call prinf('after triasymq, nqtri=*',nqtri,1)
        call prinf('after triasymq, nnodes=*',nnodes,1)
        call prin2('after triasymq, rnodes=*',rnodes,2*nnodes)
        call prin2('after triasymq, weights=*',weights,nnodes)
c
        ntri = box(15)
c
...
c
        nsource = ntri*nnodes
        jj = 0
        call prinf(' before oversampling  box (14) is *',box(14),1) 
        do j=box(14),box(14)+box(15)-1
           call triangle_area(triaflat(1,1,j),ds)
           jj = jj+1
c
        do i=1,nnodes
c
        u=rnodes(1,i)
        v=rnodes(2,i)
        source_interp(1,i,jj)=triaflat(1,1,j)+u*(triaflat(1,2,j)
     $     -triaflat(1,1,j))+v*(triaflat(1,3,j)-triaflat(1,1,j))
        source_interp(2,i,jj)=triaflat(2,1,j)+u*(triaflat(2,2,j)
     $     -triaflat(2,1,j))+v*(triaflat(2,3,j)-triaflat(2,1,j))
        source_interp(3,i,jj)=triaflat(3,1,j)+u*(triaflat(3,2,j)
     $     -triaflat(3,1,j))+v*(triaflat(3,3,j)-triaflat(3,1,j))
        
        if( ifsingle .eq. 1 ) then
        charge_interp(1,i,jj)=charge(1,j)*weights(i)*ds*2
        charge_interp(2,i,jj)=charge(2,j)*weights(i)*ds*2
        charge_interp(3,i,jj)=charge(3,j)*weights(i)*ds*2
        endif
        if( ifdouble .eq. 1 ) then
        dipstr_interp(1,i,jj)=dipstr(1,j)*weights(i)*ds*2
        dipstr_interp(2,i,jj)=dipstr(2,j)*weights(i)*ds*2
        dipstr_interp(3,i,jj)=dipstr(3,j)*weights(i)*ds*2
        dipvec_interp(1,i,jj)=dipvec(1,j)
        dipvec_interp(2,i,jj)=dipvec(2,j)
        dipvec_interp(3,i,jj)=dipvec(3,j)
        endif
        enddo
        enddo
      ...
      if( ifptfrctarg .eq. 1 .or. ifhesstarg .eq. 1 ) then
         do j=box1(16),box1(16)+box1(17)-1
            do icomp = 1,3
               call intkerbc(icomp,rlame,
     1            source_interp,ifsingle,charge_interp,
     2         ifdouble,dipstr_interp,dipvec_interp,nsource,
...
c
c
      subroutine l3dtriamindlinalloc(wlists,iaddr,nboxes,lmptot,
     1            nterms,nexptotp)
      implicit none
...
c
c
      subroutine l3dtriareorderv2(nsource,source,
