...
c
c     lfmm3dtrilhessiter - Laplace FMM in R^3: evaluate all pairwise triangle
c         interactions (including self-interaction)
c
c     lfmm3dtrilhesstargiter - Laplace FMM in R^3: evaluate all pairwise
...
c
c
        subroutine lfmm3dtrilhessiter(ier,iprec,nsource,
...
c
        ...
        ifhesstarg=0
c
        call lfmm3dtrilhesstargiter(ier,iprec,nsource,
     $     triaflat,trianorm,source,
     $     ifcharge,charge,ifdipole,dipstr,dipvec,
     $     ifpot,pot,iffld,fld,ifhess,hess,
...
c
c
        subroutine lfmm3dtrilhesstargiter(ier,iprec,nsource,
     $     triaflat,trianorm,source,
     $     ifcharge,charge,ifdipole,dipstr,dipvec,
     $     ifpot,pot,iffld,fld,ifhess,hess,
     $     ntarget,target,ifpottarg,pottarg,iffldtarg,fldtarg,
...
c
        ...
        ifevalloc=0
c
        call lfmm3dtrilhesstargiter_main(ier,iprec,
     $     ifevalfar,ifevalloc,
...
c
c
        subroutine lfmm3dtrilhesstargiter_main(ier,iprec,
     $     ifevalfar,ifevalloc,
...
ccc            if (ifcharge .eq. 1) write(*,*) 'charge'
ccc            if (ifdipole .eq. 1) write(*,*) 'dipole'
 	    if (ifcharge .eq. 1) call prinf('charge=*',ifcharge,0)
            if (ifdipole .eq. 1) call prinf('dipole=*',ifdipole,0)
        endif
c
...
c
 	    if (ifcharge .eq. 1) then
               call l3dformmptris_linear3_add(ier,scale(level),
...
c
            if (ifdipole .eq. 1 ) then
               call l3dformmptrid_linear3_add(ier,scale(level),
...
c
               if( ifcharge .eq. 1 ) then
                  call l3dformtatris_linear2_add(ier,scale(level1),
     ...
     $              wlege,nlege)
               ...
               if( ifdipole .eq. 1 ) then
                  call l3dformtatrid_linear2_add(ier,scale(level1),
     1               triaflatsort(1,1,box(14)),trianormsort(1,box(14)),
     2               dipstrsort(1,box(14)),dipvecsort(1,1,box(14)),
     $               npts,center1,
...
c
            ...
            call lfmm3dtrilhessiter_direct(nqtri,box,box1,
...
c     $              rmlexp(iaddr(2,ibox)),nterms(level),sourcesort(1,j),
c     $              ptemp,iffld,ftemp,ifhess,htemp,
c     $              ier)
...
c               call l3dtaevalhess(scale(level),center0,
c     $              rmlexp(iaddr(2,ibox)),nterms(level),targetsort(1,j),
c     $              ptemp,iffldtarg,ftemp,ifhesstarg,htemp,
c     $              ier)
               call l3dtaevalhessd_trunc(scale(level),center0,
     $              rmlexp(iaddr(2,ibox)),nterms(level),targetsort(1,j),
     $              ptemp,iffldtarg,ftemp,ifhesstarg,htemp,
     $              scarray_local,wlege,nlege)
               if (ifpottarg .eq. 1) pottarg(j)=pottarg(j)+ptemp
               if (iffldtarg .eq. 1) then
...
c
        call lfmm3dtrilhessiter_direct_self(nqtri,box,
     $     triaflatsort,trianormsort,sourcesort,
     $     ifcharge,chargesort,ifdipole,dipstrsort,dipvecsort,
     $     ifpot,pot,iffld,fld,ifhess,hess,
     $     targetsort,ifpottarg,pottarg,iffldtarg,fldtarg,
...
c    
            call lfmm3dtrilhessiter_direct(nqtri,box1,box,
     $         triaflatsort,trianormsort,sourcesort,
     $         ifcharge,chargesort,ifdipole,dipstrsort,dipvecsort,
     $         ifpot,pot,iffld,fld,ifhess,hess,
     $         targetsort,ifpottarg,pottarg,iffldtarg,fldtarg,
...
c
c
        subroutine lfmm3dtrilhessiter_direct(nqtri,box,box1,
     $     triaflat,trianorm,
...
c
c
        subroutine lfmm3dtrilhessiter_direct_self(nqtri,box,
     $     triaflat,trianorm,
     $     source,ifcharge,charge,ifdipole,dipstr,dipvec,
     $     ifpot,pot,iffld,fld,ifhess,hess,
     $     target,ifpottarg,pottarg,iffldtarg,fldtarg,
...
c
c
        subroutine l3dformmptrisone_linear2
     $     (ier, scale, triang,
     $     charge, x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
c
...
c---------------------------------------------------------------------------
c
        real *8  triang(3,3), x0y0z0(3), wlege(*)
        real *8  scale
...
c
c
        subroutine l3dformmptris_linear2
     $     (ier, scale, triang, charge, 
     1     ntri, x0y0z0, norder, nterms, mpole, mptemp, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
c
...
c---------------------------------------------------------------------------
c
        real *8  triang(3,3,1), x0y0z0(3), scale, wlege(*)
        complex *16  charge(3,1)
        complex *16  mpole(0:nterms,-nterms:nterms), mptemp(1)
c
        do i = 1,ntri
        call l3dformmptrisone_linear2(ier, scale,triang(1,1,i),
     $     charge(1,i), x0y0z0, norder, nterms, mptemp, wlege, nlege)
...
c
c
        subroutine l3dformmptris_linear2_add
     $     (ier, scale, triang, charge, 
...
c
c
        subroutine l3dformmptridone_linear2
     $     (ier, scale, triang, trinorm, dipstr, dipvec,
     $     x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
     ...
     $     (ier,scale,zparts,ctemp,dtemp,nnodes,x0y0z0,
     1     nterms,nterms,mpole,wlege,nlege)
c
        return
        end
...
c
c
        subroutine l3dformmptrid_linear2
     $     (ier,scale,triang,trinorm,dipstr,dipvec,
     1     ntri,x0y0z0,norder,nterms,mpole,mptemp,wlege,nlege)
        implicit real *8 (a-h,o-z)
c
...
c
        ...
        real *8  dipvec(3,3,1), wlege(*)
        complex *16  eye, dipstr(3,1)
        complex *16  mpole(0:nterms,-nterms:nterms), mptemp(1)
c       
        do i = 1,ntri
        call l3dformmptridone_linear2(ier,scale,triang(1,1,i),
...
c
c
        subroutine l3dformmptrid_linear2_add
     $     (ier,scale,triang,trinorm,dipstr,dipvec,
     1     ntri,x0y0z0,norder,nterms,mpole,wlege,nlege)
        implicit real *8 (a-h,o-z)
c
...
c
c     FORMMP routines: Individual triangle to multipole expansion
c     real-valued charges and dipoles
c
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c
        subroutine l3dformmptrisone_linear3
     $     (ier, scale, triang,
     $     charge, x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
        ...
        real *8  w(20)
c
        real *8 cval, charge(3), ctemp(500)
        complex *16 mpole(0:nterms,-nterms:nterms)
c       
        data eye/(0.0d0,1.0d0)/
c
...
c
     ...
     $     u*(charge(2)-charge(1))+v*(charge(3)-charge(1))
        ...
        call l3dformmp_charge_trunc
     $     (ier,scale,zparts,ctemp,nnodes,x0y0z0,
...
c
c
        subroutine l3dformmptris_linear3
     $     (ier, scale, triang, charge, 
     1     ntri, x0y0z0, norder, nterms, mpole, mptemp, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
        ...
        complex *16  charge(3,1)
        complex *16  mpole(0:nterms,-nterms:nterms), mptemp(1)
        real *8 charge_real(3)
c
        do i = 1,ntri
        charge_real(1) = charge(1,i)
        charge_real(2) = charge(2,i)
        charge_real(3) = charge(3,i)
        call l3dformmptrisone_linear3(ier, scale,triang(1,1,i),
     $     charge_real, x0y0z0, norder, nterms, mptemp, wlege, nlege)
        call l3dadd(mptemp,mpole,nterms)
        enddo
        return
        end
c
c
        subroutine l3dformmptris_linear3_add
     $     (ier, scale, triang, charge, 
     1     ntri, x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
        ...
        charge_real(3) = charge(3,i)
        call l3dformmptrisone_linear3(ier, scale,triang(1,1,i),
     $     charge_real, x0y0z0, norder, nterms, mptemp,
     $     wlege, nlege)
        call l3dadd(mptemp,mpole,nterms)
        enddo
        return
...
c
c
        subroutine l3dformmptridone_linear3
     $     (ier, scale, triang, trinorm, dipstr, dipvec,
     $     x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
c     Form multipole expansion due to DLP on single triangle.
c       ... assume that dipole strength is real-valued
c
c
c     INPUT:
c
...
c       ... assume that dipole strength is real-valued
c
        dtemp(1,i)=dtemp(1,i)*dble(ctemp(i))
        dtemp(2,i)=dtemp(2,i)*dble(ctemp(i))
        dtemp(3,i)=dtemp(3,i)*dble(ctemp(i))
         
        enddo
c
c        call l3dformmp_dp_trunc
c     $     (ier,scale,zparts,ctemp,dtemp,nnodes,x0y0z0,
c     1     nterms,nterms,mpole,wlege,nlege)
c
        call l3dformmp_dipole_trunc
     $     (ier,scale,zparts,dtemp,nnodes,x0y0z0,
     1     nterms,mpole,wlege,nlege)
c
        return
        end
...
c
c
        subroutine l3dformmptrid_linear3
     $     (ier,scale,triang,trinorm,dipstr,dipvec,
     1     ntri,x0y0z0,norder,nterms,mpole,mptemp,wlege,nlege)
        implicit real *8 (a-h,o-z)
c
...
c       
        do i = 1,ntri
        call l3dformmptridone_linear3(ier,scale,triang(1,1,i),
     1     trinorm(1,i),dipstr(1,i),dipvec(1,1,i),
     $     x0y0z0, norder,nterms,mptemp,wlege,nlege)
        call l3dadd(mptemp,mpole,nterms)
        enddo
...
c
c
        subroutine l3dformmptrid_linear3_add
     $     (ier,scale,triang,trinorm,dipstr,dipvec,
     1     ntri,x0y0z0,norder,nterms,mpole,wlege,nlege)
        implicit real *8 (a-h,o-z)
c
...
c
c
        subroutine l3dformtatrisone_linear2
     $     (ier, scale, triang,
     $     charge, x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
c
        subroutine l3dformtatris_linear2
     $     (ier, scale, triang, charge, 
     1     ntri, x0y0z0, norder, nterms, mpole, mptemp, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
        do i = 1,ntri
        call l3dformtatrisone_linear2(ier, scale,triang(1,1,i),
     $     charge(1,i), x0y0z0, norder, nterms, mptemp, wlege, nlege)
        call l3dadd(mptemp,mpole,nterms)
        enddo
        return
...
c
c
        subroutine l3dformtatris_linear2_add
     $     (ier, scale, triang, charge, 
     1     ntri, x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
c
        subroutine l3dformtatridone_linear2
     $     (ier, scale, triang, trinorm, dipstr, dipvec,
     $     x0y0z0, norder, nterms, mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c
c
        subroutine l3dformtatrid_linear2
     $     (ier,scale,triang,trinorm,dipstr,dipvec,
     1     ntri,x0y0z0,norder,nterms,mpole,mptemp, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
...
c       
        do i = 1,ntri
        call l3dformtatridone_linear2(ier,scale,triang(1,1,i),
     1     trinorm(1,i),dipstr(1,i),dipvec(1,1,i),
     $     x0y0z0, norder,nterms,mptemp, wlege, nlege)
        call l3dadd(mptemp,mpole,nterms)
        enddo
...
c
c
        subroutine l3dformtatrid_linear2_add
     $     (ier,scale,triang,trinorm,dipstr,dipvec,
     1     ntri,x0y0z0,norder,nterms,mpole, wlege, nlege)
        implicit real *8 (a-h,o-z)
c
