...
C$        t1=omp_get_wtime()
        do i=1,ntimes
        call green3elt_eval(rlam,rmu,source,du,rnorm,
     $     target,fvec,ifstrain,strain)
        enddo
        t2=second()
C$        t2=omp_get_wtime()
        call prin2('green3elt_eval: speed, particles/sec=*',
     $     ntimes/(t2-t1),1)
        ...
        call prin2('after green3elt_eval, fvec=*',fvec,3)
        call prin2('after green3elt_eval, strain=*',strain,3*3)
