...
c     in the lower half space with zero stress boundary condition at z=0
c
c     Check Okada's solution
c
...
C$        t1=omp_get_wtime()
     ...
     1     target,fvec,strain,numfunev)
        ...
        t2=second()
C$        t2=omp_get_wtime()
        call prin2('elth3triaadap: speed, triangles/sec=*',
     $     ntimes*ntri/(t2-t1),1)
...
c
        ...
        enddo
        er=sqrt(er)
        call prin2('L2 error in displacement=*',er,1)
        ...
        enddo
        er=sqrt(er)
        call prin2('L2 error in strain=*',er,1)

        do i=1,3
        fvec0(i)=fvec0(i)-fvec(i)
        enddo
        call prin2('error in displacement=*',fvec0,3)

        do i=1,3
        do j=1,3
        strain0(i,j)=strain0(i,j)-strain(i,j)
        enddo
        enddo
        call prin2('error in strain=*',strain0,3*3)
