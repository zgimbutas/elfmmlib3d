...
c
        ...
        if( itarget_type .eq. 2 ) then

        ir = 17
        open (unit = ir,file='gensta3m_1.pnt')
        read(ir,*) ntargs
        do i=1,ntargs
        read(ir,*) target(1,i),target(2,i),target(3,i)
        enddo
        ...
        dz=0
        do i=1,ntargs
        target(1,i) = target(1,i)*scale + dx
        target(2,i) = target(2,i)*scale + dy
        target(3,i) = target(3,i)*scale + dz
        enddo

c        ir = 18
c        open (unit = ir,file='gensta3m_1.out')
c        write(ir,*) ntargs
c        do i=1,ntargs
c        write(ir,*) target(1,i),target(2,i),target(3,i)
c        enddo
...
c       It dones't actually do teh FMM calculation (except for tree
c       generation.)
c
        icomp_type = 4
        t1=second()
...
ccc        open (unit = ir,file='genrect2m_1.a.tri')

ccc        open (unit = ir,file='./square.a.tri')
