...
c
        ...
        call prin2('=== Point source (free space) ===*',fvec,0)
...
C$        t1=omp_get_wtime()
        do i=1,ntimes
        call green3elu_eval(rlam,rmu,source,du,
     $     target,fvec,ifstrain,strain)
        enddo
        t2=second()
C$        t2=omp_get_wtime()
        call prin2('green3elu_eval: speed, particles/sec=*',
     $     ntimes/(t2-t1),1)
        ...
        call prin2('after green3elu_eval, fvec=*',fvec,3)
        call prin2('after green3elu_eval, strain=*',strain,3*3)
...
c       
        er=0
        do i=1,3
        er=er+(fvec0(i)-fvec(i))**2
        enddo
        er=sqrt(er)
        call prin2('error in displacement=*',er,1)

        er=0
        do i=1,3
        do j=1,3
        er=er+(strain0(i,j)-strain(i,j))**2
        enddo
        enddo
        er=sqrt(er)
        call prin2('error in strain=*',er,1)
...
c       
        eps=1e-12
        call tria3adam(ier,vert1,vert2,vert3,fun3elu_eval,nfuns,
...
c
c
        subroutine fun3elu_eval(x,y,z,target,par,f)
        implicit real *8 (a-h,o-z)
